...
const createToken = (id, role = 'aluno') => jwt.sign({ role }, process.env.JWT_SECRET, { subject: id, expiresIn: '7d' });
const membershipCode = (role = 'aluno') => {
  const prefix = role === 'admin' ? 'OMEGA-ADM' : role === 'personal' ? 'OMEGA-PT' : 'OMEGA';
...
app.post('/api/auth/register', async (req, res, next) => {
  const { nome, email, senha, idade, peso, altura, role = 'aluno', cref, codigoAcesso } = req.body;
  const sanitizedEmail = email?.trim().toLowerCase();
  const sanitizedNome = nome?.trim();
  const sanitizedRole = ['aluno', 'personal', 'admin'].includes(role) ? role : 'aluno';
  ...
  // Validação de Chave Institucional para Personal e Administrador
  if (sanitizedRole === 'personal') {
    const personalKey = process.env.PERSONAL_REGISTRATION_KEY || 'omega-personal-2026';
    if (!codigoAcesso || codigoAcesso.trim() !== personalKey.trim()) {
      return res.status(403).json({ message: 'Código de autorização de Personal Trainer incorreto ou não informado.' });
    }
  } else if (sanitizedRole === 'admin') {
    const adminKey = process.env.ADMIN_REGISTRATION_KEY || 'omega-admin-master-99';
    if (!codigoAcesso || codigoAcesso.trim() !== adminKey.trim()) {
      return res.status(403).json({ message: 'Chave mestra de Administrador incorreta ou não informada.' });
    }
  }

  const planoPadrao = sanitizedRole === 'admin'
    ? 'Administrador do Sistema'
    : sanitizedRole === 'personal'
    ? 'Acesso Profissional (Personal)'
    : 'Plano Iniciante (Mensal)';

  try {
    const passwordHash = await bcrypt.hash(senha, 12);
    const result = await pool.query(
      `INSERT INTO users (name, email, password_hash, role, cref, age, weight_kg, height_m, membership_code, plan_name)
      ...
      [
        ...
        membershipCode(sanitizedRole),
        planoPadrao,
      ]
    );
    const user = publicUser(result.rows[0]);
    res.status(201).json({ token: createToken(user.id, user.role), user });
  } catch (error) {
...
app.post('/api/auth/login', async (req, res, next) => {
  ...
  try {
    ...
    res.json({ token: createToken(safeUser.id, safeUser.role), user: safeUser });
  } catch (error) { next(error); }
});
...
app.get('/api/admin/users', authRequired, roleRequired('admin'), async (_req, res, next) => {
  try {
