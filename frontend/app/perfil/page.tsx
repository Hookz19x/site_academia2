...
export default function PerfilPage() {
  ...
  const [fotoPerfil, setFotoPerfil] = useState(() => typeof window === 'undefined' ? '/avatar-padrao.png' : localStorage.getItem('@omegaGym:fotoPerfil') ?? '/avatar-padrao.png');
  ...
  function alterarFoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const reader = new FileReader(); reader.onloadend = () => { const foto = reader.result as string; setFotoPerfil(foto); localStorage.setItem('@omegaGym:fotoPerfil', foto); }; reader.readAsDataURL(arquivo);
  }
  function sair() { encerrarSessao(); localStorage.removeItem('@omegaGym:fotoPerfil'); window.location.href = '/login'; }
